...
std::string HelpMessage(HelpMessageMode mode)                                   // MCHN
{
    ...
    strUsage += "  -conf=<file>           " + strprintf(_("Specify configuration file (default: %s)"), "beidouchain.conf") + "\n";
...
#ifndef WIN32
    strUsage += "  -pid=<file>            " + strprintf(_("Specify pid file (default: %s)"), "beidouchain.pid") + "\n";
#endif
    strUsage += "  -reindex               " + _("Rebuild the blockchain and reindex transactions on startup.") + "\n";
#if !defined(WIN32)
    strUsage += "  -sysperms              " + _("Create new files with system default permissions, instead of umask 077 (only effective with disabled wallet functionality)") + "\n";
    strUsage += "  -shortoutput           " + _("Returns connection string if this node can start or default beidouchain address otherwise") + "\n";
#endif
/* MCHN START */    
/* Default was 0 */    
...
/* MCHN END */    
    ...
    strUsage += "\n" + _("BeidouChain runtime parameters") + "\n";    
    strUsage += "  -initprivkey=<privkey>                   " + _("Manually set the wallet default address and private key when running beidouchaind for the first time.") + "\n";
    strUsage += "  -handshakelocal=<address>                " + _("Manually override the wallet address which is used for handshaking with other peers in a BeidouChain blockchain.") + "\n";
    strUsage += "  -hideknownopdrops=<n>                    " + strprintf(_("Remove recognized BeidouChain OP_DROP metadata from the responses to JSON_RPC calls (default: %u)"), 0) + "\n";
...
{
    return FormatParagraph(_("Copyright (c) Huasystem Ltd - www.beidouchain.com")) + "\n" +
           "\n" +
           ...
           "\n" +
           FormatParagraph(_("Full terms are shown at: http://www.beidouchain.com/terms-of-service/")) +
           "\n";
...
/* MCHN END */    
    ...
    // Sanity check
    if (!InitSanityCheck())
        return InitError(_("Initialization sanity check failed. BeidouChain Core is shutting down."));
...
// Anyway, we don't need this lock as we have permission db lock    
    ...
    if (!lock.try_lock())
        return InitError(strprintf(_("Cannot obtain a lock on data directory %s. BeidouChain Core is probably already running."), strDataDir));
#endif
/* MCHN END */
...
/* MCHN START */    
    if(mc_gState->m_NetworkParams->IsProtocolBeidouchain())
    {
        LogPrintf("BeidouChain version %s (%s)\n", mc_gState->GetFullVersion(), CLIENT_DATE);
    }
...
/* MCHN START*/    
    uiInterface.InitMessage(_("Initializing beidouchain..."));
    ...
    {        
           ...
           || (mc_gState->m_NetworkParams->m_Status == MC_PRM_STATUS_MINIMAL))
        {
            if(mc_gState->m_NetworkParams->m_Status == MC_PRM_STATUS_MINIMAL)
            {
                ...
                if(init_privkey.size())
                {
                    if(seed_attempt == 1)
                    {
                        ...
                        {
                            return InitError(_("The initprivkey runtime parameter can only be used when connecting to BeidouChain 1.0 beta 2 or later"));                                                        
                        }
            ...
            if(mc_QuerySeed(seedThreadGroup,seed_node))
            {
                ...
                {
                    seed_error="Couldn't disconnect from the seed node, please restart beidouchaind";
                }
                else
                {
                    if(seed_port.size() == 0)
                    ...
                    {
                        seed_error=strprintf("Couldn't connect to the seed node %s on port %s - please check beidouchaind is running at that address and that your firewall settings allow incoming connections.",                
                ...
                (mc_gState->GetProtocolVersion() < (int)mc_gState->m_NetworkParams->GetInt64Param("protocolversion")) )
            {
                seed_error=strprintf("Couldn't connect to the seed node %s on port %s.\n"
                            "Blockchain was created by beidouchaind with newer protocol version (%d)\n"                
                            "Please upgrade to the latest version of BeidouChain or connect only to blockchains using protocol version %d or earlier.\n",                
            ...
            {                                
                ...
                {
                    seed_error=strprintf("The protocol version (%d) for blockchain %s has been deprecated and was last supported in BeidouChain 1.0 beta 1\n",                
            ...
            if(mc_gState->m_NetworkParams->m_Status == MC_PRM_STATUS_VALID)
            {
                SelectBeidouChainParams(mc_gState->m_Params->NetworkName());
        ...
        else
        {
            if(mc_gState->m_NetworkParams->m_Status == MC_PRM_STATUS_GENERATED)
            {
                ...
                mc_gState->m_NetworkParams->SetGlobals();                           // Needed to update IsProtocolBeidouchain flag in case of bitcoin
    ...
    if(mc_gState->m_NetworkParams->m_Status == MC_PRM_STATUS_VALID)
    {
        ...
        {
            seed_error=strprintf("ERROR: Couldn't initialize asset database for blockchain %s. Please restart beidouchaind with reindex=1.\n",mc_gState->m_Params->NetworkName());
        ...
        else
        {
            ...
            if(mc_gState->m_WalletMode != MC_WMD_NONE)
            {
                ...
                {
                    return InitError("Wallet tx database corrupted. Please restart beidouchaind with -rescan\n");                        
                }
                ...
                {
                    return InitError(strprintf("Wallet tx database was created with different wallet version (%d). Please restart beidouchaind with reindex=1 \n",pwalletTxsMain->m_Database->m_DBStat.m_WalletVersion));                        
                }        

                if((pwalletTxsMain->m_Database->m_DBStat.m_InitMode & MC_WMD_MODE_MASK) != (mc_gState->m_WalletMode & MC_WMD_MODE_MASK))
                {
                    return InitError(strprintf("Wallet tx database was created in different mode (%08X). Please restart beidouchaind with reindex=1 \n",pwalletTxsMain->m_Database->m_DBStat.m_InitMode));                        
                }        
            }
        }
        ...
        if(pwalletMain == NULL)                                                 // Opening wallet only after beidouchain parameters were initizalized
        {
            pwalletMain = new CWallet(strWalletFile);
            DBErrors nLoadWalletRetForBuild = pwalletMain->LoadWallet(fFirstRunForBuild);
            ...
            if(pwalletMain)
            {
                ...
                if(pwalletMain->vchDefaultKey.IsValid())
                {
                    ...
                    {    
                        ...
                        sprintf(bufOutput,"beidouchain-cli %s grant %s connect\n",mc_gState->m_NetworkParams->Name(),
                             CBitcoinAddress(pwalletMain->vchDefaultKey.GetID()).ToString().c_str());
                        bytes_written=write(OutputPipe,bufOutput,strlen(bufOutput));
                        sprintf(bufOutput,"beidouchain-cli %s grant %s connect,send,receive\n\n",mc_gState->m_NetworkParams->Name(),
                             CBitcoinAddress(pwalletMain->vchDefaultKey.GetID()).ToString().c_str());
                        bytes_written=write(OutputPipe,bufOutput,strlen(bufOutput));
                    }
                    else
...
/* MCHN START */    
    ...
    {    
        ...
        bytes_written=write(OutputPipe,bufOutput,strlen(bufOutput));
        sprintf(bufOutput,"beidouchaind %s:%d\n\n",BeidouchainServerAddress().c_str(),GetListenPort());
        bytes_written=write(OutputPipe,bufOutput,strlen(bufOutput));
        if(found_ips > 1)
        {
...
/* MCHN START */                
                bool fRet = uiInterface.ThreadSafeMessageBox(
                    strLoadError + ".\n\n" + _("Please restart beidouchaind with reindex=1."),
    ...
    if (fDisableWallet) {
        ...
        if (nLoadWalletRet != DB_LOAD_OK)
        {
            ...
            else if (nLoadWalletRet == DB_NONCRITICAL_ERROR)
            {
                ...
                strErrors << _("Error loading wallet.dat: Wallet requires newer version of BeidouChain Core") << "\n";
            else if (nLoadWalletRet == DB_NEED_REWRITE)                         // MCHN
            {
                strErrors << _("Wallet needed to be rewritten: restart BeidouChain Core to complete") << "\n";
