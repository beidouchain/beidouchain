...
bool AcceptBeidouChainTransaction(const CTransaction& tx, 
...
bool ProcessBeidouchainVerack(CNode* pfrom, CDataStream& vRecv,bool fIsVerackack,bool *disconnect_flag);
bool PushBeidouChainVerack(CNode* pfrom, bool fIsVerackack);
bool BeidouchainNode_CanConnect(CNode *pnode);
bool BeidouchainNode_DisconnectRemote(CNode *pnode);
bool BeidouchainNode_DisconnectLocal(CNode *pnode);
bool BeidouchainNode_RespondToGetData(CNode *pnode);
bool BeidouchainNode_SendInv(CNode *pnode);
bool BeidouchainNode_AcceptData(CNode *pnode);
bool BeidouchainNode_IgnoreIncoming(CNode *pnode);
bool BeidouchainNode_IsLocal(CNode *pnode);
...
const string strMessageMagic = "BeidouChain Signed Message:\n";
...
int BeidouchainNode_ApplyUpgrades()
{
...
void BeidouchainNode_UpdateBlockByHeightList(CBlockIndex *pindex)
{
    if(pindex->nHeight < 0)
    {
        return;
...
bool BeidouchainNode_IsBlockChainSynced(CNode *pnode)
{
...
/* MCHN START */
        ...
        if(mc_gState->m_Features->Streams())
        {
            if(!AcceptBeidouChainTransaction(tx,view,-1,true,reason, &replay))
            {
                return state.DoS(0,
                                 error("AcceptToMemoryPool: : AcceptBeidouChainTransaction failed %s : %s", hash.ToString(),reason),
                                 REJECT_NONSTANDARD, reason);
            }
        }
        else
...
/* MCHN START */        
    if (block.GetHash() == Params().HashGenesisBlock()){        
        ...
        for (unsigned int i = 0; i < block.vtx.size(); i++)
        {
            ...
            if(mc_gState->m_Features->Streams())
            {
                if(!AcceptBeidouChainTransaction(tx,view,offset,true,reason,NULL))
                {
...
/* MCHN START */        
            ...
            if(!fJustCheck)
            {
                if(mc_gState->m_Features->Streams())
                {
                    ...
                    {
                        return state.DoS(0,
                                         error("ConnectBlock: : AcceptBeidouChainTransaction failed %s : %s", tx.GetHash().ToString(),reason),
                                         REJECT_NONSTANDARD, reason);
                    }
                }
                else
    ...
    for (unsigned int i = 0; i < block.vtx.size(); i++)
    {
        ...
        if (tx.IsCoinBase())
        {
            ...
            if(!fJustCheck)
            {
                if(mc_gState->m_Features->Streams())
                {
                    if(!AcceptBeidouChainTransaction(tx,view,coinbase_offset,true,reason,NULL))
                    {
                        return false;       
                    }
                }
...
CBlockIndex* AddToBlockIndex(const CBlockHeader& block)
{
    ...
    BeidouchainNode_UpdateBlockByHeightList(pindexNew);
...
bool AcceptBlockHeader(const CBlockHeader& block, CValidationState& state, CBlockIndex** ppindex, int node_id)
{
    ...
    if( (node_id > 0) && (pindexPrev != NULL) )
    {
        ...
        {
            BeidouchainNode_UpdateBlockByHeightList(pindexPrev);
        }
        lpNextForPrev=NULL;
...
/* MCHN START */    
    if(!BeidouchainNode_RespondToGetData(pfrom)) 
    {
...
/* MCHN START */
        ...
        {            
            ...
            {
                PushBeidouChainVerack(pfrom,false);
            }
        }
        else
        {
...
/* MCHN START */
        if(mc_gState->m_NetworkParams->IsProtocolBeidouchain())
        {
            ...
            else
            {
                ...
                else
                {
                    ...
                    if(!ProcessBeidouchainVerack(pfrom,vRecv,false,&disconnect_flag))
                    {
                    ...
                    else
                    {
                        ...
                        {
                            pfrom->fDisconnect = !PushBeidouChainVerack(pfrom,true);
                        }
                        else
                        {
    ...
    else if (strCommand == "verackack")
    {
        if(mc_gState->m_NetworkParams->IsProtocolBeidouchain())
        {
            ...
            if(!ProcessBeidouchainVerack(pfrom,vRecv,true,&disconnect_flag))
            {
            ...
            {
                pfrom->fDisconnect |= !PushBeidouChainVerack(pfrom,false);       // MCHN |= to avoid setting fDisconnect to false
            }
            if(pfrom->fDisconnect)
            {
                mc_gState->m_NetworkState = MC_NTS_SEED_READY;
...
/* MCHN END */
    ...
    else if (strCommand == "inv")
    {
        ...
        for (unsigned int nInv = 0; nInv < vInv.size(); nInv++)
        {
            ...
            if (inv.type == MSG_BLOCK) {
                ...
                if (!fAlreadyHave && !fImporting && !fReindex && !mapBlocksInFlight.count(inv.hash)) {
                    ...
                    if(!BeidouchainNode_IgnoreIncoming(pfrom))
                    {
        ...
        if (!vToFetch.empty() && !BeidouchainNode_IgnoreIncoming(pfrom))      // MCHN
    ...
    else if (strCommand == "tx" && BeidouchainNode_AcceptData(pfrom))            // MCHN
    {
/* MCHN START */        
        if(!BeidouchainNode_IgnoreIncoming(pfrom))
        {
    ...
    else if (strCommand == "headers" && !fImporting && !fReindex) // Ignore headers received while importing
    {
        ...
        if (nCount == MAX_HEADERS_RESULTS && pindexLast) {
            ...
            pfrom->PushMessage("getheaders", chainActive.GetLocator(pindexLast), uint256(0));
                                                                                ...
                                                                                // MCHN
    else if (strCommand == "block" && !fImporting && !fReindex && BeidouchainNode_AcceptData(pfrom)) // Ignore blocks received while importing
    {
        CBlock block;
        vRecv >> block;
/* MCHN START */        
...
/* MCHN END */        
    ...
    else if (strCommand == "mempool" && BeidouchainNode_SendInv(pfrom))          // MCHN
    {
...
/* MCHN START */    
    {        
        ...
        if(!pfrom->fDisconnect)
        {
            if(BeidouchainNode_DisconnectRemote(pfrom))
            {            
                    pfrom->fDisconnect=true;
            ...
            if(!pfrom->fDisconnect)
            {
                if(pfrom->fCanConnectLocal && BeidouchainNode_DisconnectLocal(pfrom))
                {
                        pfrom->fDisconnect=true;
...
/* MCHN END */            
        ...
        {
/* MCHN START */            
            if(pfrom->fDisconnect || !BeidouchainNode_DisconnectRemote(pfrom))
            {
/* MCHN END */            
...
*/
                ...
                if (pto->setInventoryKnown.insert(inv).second)
                {
                    ...
                    if (vInv.size() >= 1000)
                    {
                        if(BeidouchainNode_SendInv(pto))                         // MCNN
...
/* MCHN START */        
        ...
        {
            LOCK(cs_main);
            ignore_incoming=BeidouchainNode_IgnoreIncoming(pto);
        }
        if(!ignore_incoming)
        {
...
/* MCHN START */        
        {
            LOCK(cs_main);
            CNode* seed_node;
            if(!pto->fSyncedOnce && BeidouchainNode_IsBlockChainSynced(pto))
            {
                ...
                if(seed_node == pto)
                {
                    if(!BeidouchainNode_IsLocal(pto))
                    {
