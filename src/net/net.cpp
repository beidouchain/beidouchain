...
/* MCHN START */
    ...
    {
        subver=FormatSubVersion("BeidouChain", mc_gState->GetProtocolVersion(), std::vector<string>());
    }
...
void ThreadMapPort()
{
    ...
    if (r == 1)
    {
        ...
        string strDesc = "BeidouChain " + FormatFullVersion();
...
bool BindListenPort(const CService &addrBind, string& strError, bool fWhitelisted)
{
    ...
    if (::bind(hListenSocket, (struct sockaddr*)&sockaddr, len) == SOCKET_ERROR)
    {
        ...
        if (nErr == WSAEADDRINUSE)
            strError = strprintf(_("Unable to bind to %s on this computer. BeidouChain Core is probably already running."), addrBind.ToString());
        else
