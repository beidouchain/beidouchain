...
/* MCHN START */    
// TODO too many duplicate code with ListWalletTransactions and may be AccepBeidouChainTransaction
...
/* BCHN START */
    ...
    Array vlocation;
    entry.push_back(Pair("location", vlocation));
...
Value appendrawchange(const Array& params, bool fHelp)
{
    ...
    {
        if(ParseBeidouchainTxOutToBuffer(0,txout,asset_amounts,lpScript,&allowed,&required,strError))
        {
    ...
    {
        if(ParseBeidouchainTxOutToBuffer(tx.vin[i].prevout.hash,txout,asset_amounts,lpScript,&allowed,&required,strError))
        {
            nAmount+=txout.nValue;
        }
        i++;
...
Value disablerawtransaction(const Array& params, bool fHelp)
{
    ...
    {
        throw JSONRPCError(RPC_NOT_SUPPORTED, "disablerawtransaction supported only for protocol=beidouchain");
    }    
