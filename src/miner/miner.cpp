            ...
            if(canMine)
            {
                ...
                *canMine=mc_gState->m_Permissions->CanMine(NULL,pubkey_hash);
                if((*canMine & MC_PTP_MINE) == 0)
                {
                    ...
                    {
                        LogPrintf("mchn: BeidouChainMiner: cannot mine now, waiting...\n");
                    }
                    testValidity=false;
                }
                else
                {
                    ...
                    {
                        LogPrintf("CreateNewBlock(): total size %u\n", nBlockSize);
                        LogPrintf("mchn: BeidouChainMiner: Starting mining...\n");
                    }                    
                }
            }            
        }
...
bool ProcessBlockFound(CBlock* pblock, CWallet& wallet, CReserveKey& reservekey)
{
    ...
    // Found a solution
    {
        ...
        {
            return error("BeidouChainMiner : mining is paused, generated block is dropped");            
        }
        if (pblock->hashPrevBlock != chainActive.Tip()->GetBlockHash())
        {
            return error("BeidouChainMiner : generated block is stale");
        }
    }
    ...
    if (!ProcessNewBlock(state, NULL, pblock))
        return error("BeidouChainMiner : ProcessNewBlock, block not accepted");
...
{
    LogPrintf("BeidouChainMiner started\n");
...
/* MCHN START */            
            ...
            if (Params().MiningRequiresPeers() 
                    ...
                    || (mc_gState->m_NetworkParams->IsProtocolBeidouchain() == 0)
                    )
                    ) {
...
/* MCHN END */    
            if (!pblocktemplate.get())
            {
                LogPrintf("Error in BeidouChainMiner: Keypool ran out, please call keypoolrefill before restarting the mining thread\n");
                return;
            }
            CBlock *pblock = &pblocktemplate->block;
            IncrementExtraNonce(pblock, pindexPrev, nExtraNonce,pwallet);

            LogPrint("mcminer","mchn-miner: Running BeidouChainMiner with %u transactions in block (%u bytes)\n", pblock->vtx.size(),
            ...
            while (true) {
                ...
                if (fFound)
                {
                    if (hash <= hashTarget)
                    {
                        ...
                        LogPrintf("BeidouChainMiner: Block Found - %s, prev: %s, height: %d, txs: %d\n",
                                hash.GetHex(),pblock->hashPrevBlock.ToString().c_str(),mc_gState->m_Permissions->m_Block+1,(int)pblock->vtx.size());
/*                        
                        LogPrintf("BeidouChainMiner:\n");
                        LogPrintf("proof-of-work found  \n  hash: %s  \ntarget: %s\n", hash.GetHex(), hashTarget.GetHex());
*/                     
/* MCHN START */                        
                        if(mc_gState->m_ProtocolVersionToUpgrade > mc_gState->m_NetworkParams->ProtocolVersion())
                        {
                            LogPrintf("BeidouChainMiner: Waiting for upgrade, block is dropped\n");
                        }
                        else
                        {
...
/* MCHN END */    
    ...
    {
        LogPrintf("BeidouChainMiner terminated\n");
        throw;
    }
}
