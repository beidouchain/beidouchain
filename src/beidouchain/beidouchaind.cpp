...
bool AppInit(int argc, char* argv[])
{
    ...
    {
        fprintf(stderr,"\nError: Data directory %s needs to exist before calling beidouchaind. Exiting...\n\n",mapArgs["-datadir"].c_str());
        return false;        
    }
        ...
        (mc_gState->m_Params->NetworkName() == NULL))
    {
        ...
        {
            strUsage += "\n" + _("Usage:") + "\n" +
                  "  beidouchaind <blockchain-name> [options]                     " + _("Start BeidouChain Core Daemon") + "\n";
    ...
    is_daemon=false;
        ...
        if (fDaemon)
        {
            ...
            if(!GetBoolArg("-shortoutput", false))
            {
                fprintf(stdout, "BeidouChain server starting\n");
            }
...
//                delete mc_gState;
    ...
    if(err)
    {
        fprintf(stderr,"ERROR: Couldn't read configuration file for blockchain %s. Please try upgrading BeidouChain. Exiting...\n",mc_gState->m_Params->NetworkName());
        delete mc_gState;                
        return false;
    }
    ...
    if(err)
    {
        ...
        {
            fprintf(stderr,"\nERROR: Couldn't initialize permission database for blockchain %s. Please restart beidouchaind with reindex=1.\n",mc_gState->m_Params->NetworkName());                        
        }
        else
        {
            fprintf(stderr,"\nERROR: Couldn't initialize permission database for blockchain %s. Probably beidouchaind for this blockchain is already running. Exiting...\n",mc_gState->m_Params->NetworkName());
        }
        delete mc_gState;                
        return false;
    }
    ...
    {
            fprintf(stderr,"ERROR: Parameter set for blockchain %s was generated by BeidouChain running newer protocol version (%d)\n\n",
                    mc_gState->m_Params->NetworkName(),(int)mc_gState->m_NetworkParams->GetInt64Param("protocolversion"));                        
            fprintf(stderr,"Please upgrade BeidouChain\n\n");
            delete mc_gState;                
            return false;
    }
    ...
    {
            fprintf(stderr,"ERROR: The protocol version (%d) for blockchain %s has been deprecated and was last supported in BeidouChain 1.0 beta 1\n\n",
    ...
    switch(mc_gState->m_NetworkParams->m_Status)
    {
        ...
        case MC_PRM_STATUS_MINIMAL:
            if(mc_gState->GetSeedNode() == NULL)
            {
                ...
                fprintf(stderr,"  beidouchain-util create %s\n",mc_gState->m_Params->NetworkName());
                fprintf(stderr,"  beidouchain-util clone <old-blockchain-name> %s\n",mc_gState->m_Params->NetworkName());
                fprintf(stderr,"\nAnd rerun beidouchaind %s\n\n\n",mc_gState->m_Params->NetworkName());                        
                fprintf(stderr,"If you want to connect to existing blockchain please specify seed node:\n\n");
                fprintf(stderr,"  beidouchaind %s@<seed-node-ip>\n",mc_gState->m_Params->NetworkName());
                fprintf(stderr,"  beidouchaind %s@<seed-node-ip>:<seed-node-port>\n\n\n",mc_gState->m_Params->NetworkName());
                delete mc_gState;                
                return false;
            }
            break;
        ...
        case MC_PRM_STATUS_INVALID:
            ...
            fprintf(stderr,"  beidouchain-util clone %s <new-blockchain-name>\n",mc_gState->m_Params->NetworkName());
            delete mc_gState;                
            return false;
