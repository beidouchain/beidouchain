...
bool FindRelevantCoins(CWallet *lpWallet,                                       // IN  Wallet (to access grouping object)
                       ...
                       unsigned char *in_row,int in_size,                       // TMP temporary buffer row and its size
                       mc_Script *lpScript,                                     // TMP temporary beidouchain script object
                       int *in_special_row,                                     // IN  Coordinates of special rows
                       ...
                       std::string& strFailReason)                              // OUT error message
{
    ...
    BOOST_FOREACH(const COutput& out, vCoins)
    {
        ...
        out_i=out.i;
        tmp_amounts->Clear();
        if(ParseBeidouchainTxOutToBuffer(hash,txout,tmp_amounts,lpScript,&allowed,&required,mapSpecialEntity,strError))
        {
                        ...
                        std::string& strFailReason)                             // OUT error message
{
    ...
    {       
        ...
        if( (count<max_inputs) && (out.nDepth >= min_conf) )
        {
            ...
            out_i=out.i;
            tmp_amounts->Clear();
            if(ParseBeidouchainTxOutToBuffer(hash,txout,tmp_amounts,lpScript,&allowed,&required,strError))
            {
                      ...
                      const CCoinControl* coinControl)
{
    ...
    if(( nTargetValue > 0) || (!found_selected) )
    {
        ...
        if(!lpWallet->SelectBeidouChainCoins(nTargetValue,vCoins,in_map,in_amounts,in_special_row[0],in_asset_row,in_prefered_row,setAssetCoins, nAssetValueIn, coinControl))
        {
            return false;
        }                        
...
{   
    ...
    {
        if (nValue < 0)                                                         // Beidouchain allows protocol zero-value outputs
        {
...
//    if (vecSend.empty() || nValue < 0)
    ...
    if(vecSend.size())
    {
        ...
        BOOST_FOREACH (const PAIRTYPE(CScript, CAmount)& s, vecSend)            // Filling buffer with output amounts
        {
            ...
            if(!ParseBeidouchainTxOutToBuffer(hash,txout,out_amounts,lpScript,NULL,&this_required,&mapSpecialEntity,strFailReason))
            {
                goto exitlbl;
            }
    ...
    else
    {
        ...
        if(!ParseBeidouchainTxOutToBuffer(hash,txout,out_amounts,lpScript,NULL,&required,strFailReason))
        {
            goto exitlbl;
        }        
    }
    ...
    last_time=this_time;
                                                                                ...
                                                                                // We need this as in SelectBeidouChainCoinsMinConf coins will be shuffled
...
bool CWallet::CreateBeidouChainTransaction(const vector<pair<CScript, CAmount> >& vecSend,
                                CWalletTx& wtxNew, CReserveKey& reservekey, CAmount& nFeeRet, std::string& strFailReason, const CCoinControl* coinControl,
...
//    if(mc_gState->m_WalletMode & MC_WMD_ADDRESS_TXS)                          // Not supported, not used in this case
    ...
    if( (tmp_amounts != NULL) && (lpScript != NULL) )                       // Count assets in unspent coins
    {
        ...
        BOOST_FOREACH(const COutput& out, vCoins)
        {
            ...
            ParseBeidouchainTxOutToBuffer(hash,txout,tmp_amounts,lpScript,NULL,NULL,strError);
        }
