...
bool ParseBeidouchainTxOutToBuffer(uint256 hash,                                 // IN, tx hash, if !=0 genesis asset reference is retrieved from asset DB
...
bool ParseBeidouchainTxOutToBuffer(uint256 hash,                                 // IN, tx hash, if !=0 genesis asset reference is retrieved from asset DB
                                  ...
                                  string& strFailReason)                        // OUT error
{
    return ParseBeidouchainTxOutToBuffer(hash,txout,amounts,lpScript,allowed,required,NULL,strFailReason);
}
