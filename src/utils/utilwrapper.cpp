...
void mc_Params::Parse(int argc, const char* const argv[])
{
    ...
    if(m_NumArguments)
    {
        ...
        if((strcmp(exe_name,"beidouchain-util") == 0) || (strcmp(exe_name,"beidouchain-util.exe") == 0))
        {
            m_FirstArgumentType=MC_FAT_COMMAND;
        }
        else
        {
            if((strcmp(exe_name,"beidouchain-cli") == 0) || 
               (strcmp(exe_name,"beidouchain-cli.exe") == 0) ||                     
               (strcmp(exe_name,"beidouchaind") == 0) ||                     
               (strcmp(exe_name,"beidouchaind.exe") == 0))
            {
                ...
                if((m_FirstArgumentType == MC_FAT_NETWORK) && ((strcmp(exe_name,"beidouchaind") == 0) || (strcmp(exe_name,"beidouchaind.exe") == 0)))
                {
                    m_Arguments[m_NumArguments]=m_Arguments[0]+length;
...
{
    // Windows < Vista: C:\Documents and Settings\Username\Application Data\BeidouChain
    // Windows >= Vista: C:\Users\Username\AppData\Roaming\BeidouChain
    // Mac and Unix: ~/.beidouchain
#ifdef WIN32
    // Windows
    return GetSpecialFolderPath(CSIDL_APPDATA) / "BeidouChain";
#else
    // Mac and Unix
...
void mc_CheckDataDirInConfFile()
{
    ...
    mapConfig=new mc_MapStringString;
    if(mc_ReadGeneralConfigFile(mapConfig,NULL,"beidouchain",".conf") == 0)
    {
...
{
    mc_ReadConfigFile(mc_GetConfigFile(network_name,"beidouchain",".conf"),&mapArgs, &mapMultiArgs,"-");    
...
int mc_BeidouchainParams::SetGlobals()
{
    m_IsProtocolBeidouChain=1;
    void *ptr=GetParam("chainprotocol",NULL);
    if(ptr)
    {
        if(strcmp((char*)ptr,"beidouchain"))
        {
            m_IsProtocolBeidouChain=0;
        }
    }
...
int mc_BeidouchainParams::Import(const char *name,const char *source_address)
{
    
    return MC_ERR_NOERROR;
}

std::string BeidouchainServerAddress()
{
