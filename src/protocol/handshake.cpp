...
bool BeidouchainNode_DisconnectRemote(CNode *pnode)
{
    ...
    {
        return !BeidouchainNode_CanConnect(pnode);
    }
...
bool BeidouchainNode_AcceptData(CNode *pnode)
{
    ...
    return !BeidouchainNode_DisconnectRemote(pnode);
}
...
bool VerifyBeidouchainVerackHash(string sParameterSetHash,uint64_t nNonce)
{
...
{   
    ...
    {
        if(!VerifyBeidouchainVerackHash(sParameterSetHash,nNonce))
        {
            if(!fIsVerackack)
            {
...
//                return false;
    ...
    if(MCP_ANYONE_CAN_CONNECT == 0)
    {
        ...
        if(fIsVerackack)
        {
            ...
            if(!BeidouchainNode_CanConnect(pfrom))
            {
