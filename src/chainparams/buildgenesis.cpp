...
int mc_GenerateConfFiles(const char *network_name)
{
    ...
    fileHan=mc_OpenFile(NULL,"beidouchain",".conf","r",MC_FOM_RELATIVE_TO_DATADIR | MC_FOM_CREATE_DIR);
    if(fileHan == NULL)
    {
        fileHan=mc_OpenFile(NULL,"beidouchain",".conf","w",MC_FOM_RELATIVE_TO_DATADIR);
    }
    if(fileHan)
    {
    ...
    fileHan=mc_OpenFile(network_name,"beidouchain",".conf","r",MC_FOM_RELATIVE_TO_DATADIR | MC_FOM_CREATE_DIR);
    if(fileHan == NULL)
    {
        fileHan=mc_OpenFile(network_name,"beidouchain",".conf","w",MC_FOM_RELATIVE_TO_DATADIR);
        if(fileHan)
        {
            fprintf(fileHan,"rpcuser=beidouchainrpc\n");
...
int mc_BeidouchainParams::Build(const unsigned char* pubkey, int pubkey_size) 
{
