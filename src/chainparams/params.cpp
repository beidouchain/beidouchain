...
int mc_OneBeidouchainParam::IsRelevant(int version)
{
    int ret=1;
...
void mc_BeidouchainParams::Zero()
{
    m_lpData = NULL;
    m_lpParams = NULL;        
    m_lpIndex=NULL;    
    ...
    m_Size=0;
    m_IsProtocolBeidouChain=1;
    m_ProtocolVersion=0;
    
    m_AssetRefSize=MC_AST_SHORT_TXID_SIZE;
}

void mc_BeidouchainParams::Destroy()
{
    if(m_lpData)
    {
        mc_Delete(m_lpData);
...
int64_t mc_BeidouchainParams::GetInt64Param(const char *param)
{
    int size;
...
double mc_BeidouchainParams::GetDoubleParam(const char *param)
{
...
void* mc_BeidouchainParams::GetParam(const char *param,int* size)
{
    if(m_lpIndex == NULL)
    {
        return NULL;
...
int mc_BeidouchainParams::SetParam(const char *param,const char* value,int size)
{
    int offset;
    if(m_lpIndex == NULL)
    {
...
int mc_BeidouchainParams::SetParam(const char *param,int64_t value)
{
    int size;
    char buf[8];
    if(m_lpIndex == NULL)
...
void mc_BeidouchainParams::Init()
{
    int size,max_size,i;
    ...
    m_Count=sizeof(BeidouchainParamArray)/sizeof(mc_OneBeidouchainParam);
    max_size=0;
    
    for(i=0;i<m_Count;i++)
    {
        size=0;
        switch((BeidouchainParamArray+i)->m_Type & MC_PRM_DATA_TYPE_MASK)
        {
            case MC_PRM_BINARY  : size=(BeidouchainParamArray+i)->m_MaxStringSize;   break;
            case MC_PRM_STRING  : size=(BeidouchainParamArray+i)->m_MaxStringSize+1; break;
    ...
    m_lpParams=(mc_OneBeidouchainParam*)mc_New(sizeof(BeidouchainParamArray));
    m_lpData=(char*)mc_New(max_size);
    m_lpCoord=(int*)mc_New(2*m_Count*sizeof(int));
    
    memcpy(m_lpParams,BeidouchainParamArray,sizeof(BeidouchainParamArray));
    for(i=0;i<m_Count;i++)
    {
...
int mc_BeidouchainParams::Create(const char* name,int version)
{
    int size,offset,i,set;
    mc_OneBeidouchainParam *param;
    char *ptrData;
    int num_sets;
...
double mc_BeidouchainParams::ParamAccuracy()
{
    return 1./(double)MC_PRM_DECIMAL_GRANULARITY;
}


int mc_BeidouchainParams::Read(const char* name)
{
    return Read(name,0,NULL,0);
}

int mc_BeidouchainParams::Read(const char* name,int argc, char* argv[],int create_version)
{
    ...
    mc_BeidouchainParams *lpDefaultParams;
    ...
    lpDefaultParams = new mc_BeidouchainParams;
    ...
    for(i=0;i<m_Count;i++)
    {
        ...
        {        
            ...
            switch((BeidouchainParamArray+i)->m_Type & MC_PRM_DATA_TYPE_MASK)
            {
                ...
                case MC_PRM_INT32:
                    size=4;
                    if(((BeidouchainParamArray+i)->m_Type & MC_PRM_SOURCE_MASK) == MC_PRM_USER)
                    {
                        if(atoll(ptr) > (BeidouchainParamArray+i)->m_MaxIntegerValue)
                        {
                            printf("Invalid parameter value for %s - too high: %s\n",param->m_DisplayName,ptr);                        
                            return MC_ERR_INVALID_PARAMETER_VALUE;                                                
                        }
                        if(atoll(ptr) < (BeidouchainParamArray+i)->m_MinIntegerValue)
                        {
                            printf("Invalid parameter value for %s - too low: %s\n",param->m_DisplayName,ptr);                        
                            return MC_ERR_INVALID_PARAMETER_VALUE;                                                
                        }
                    }
                    if((BeidouchainParamArray+i)->m_Type & MC_PRM_DECIMAL)
                    {
                        double d=atof(ptr);
                        if(d >= 0)
                        {
        ...
        {
            if( ((((BeidouchainParamArray+i)->m_Type & MC_PRM_SOURCE_MASK) != MC_PRM_CALCULATED) && 
               (((BeidouchainParamArray+i)->m_Type & MC_PRM_SOURCE_MASK) != MC_PRM_GENERATED)) ||
               (argc > 0) )     
            {
                strcpy(m_lpData+offset,param->m_Name);
                offset+=strlen(param->m_Name)+1;
...
int mc_BeidouchainParams::Set(const char *name,const char *source,int source_size)
{
...
int mc_BeidouchainParams::Clone(const char* name, mc_BeidouchainParams* source)
{
    int err;
...
int mc_BeidouchainParams::CalculateHash(unsigned char *hash)
{
    int i;
    int take_it;
    mc_SHA256 *hasher;
...
int mc_BeidouchainParams::Validate()
{
    int i,size,offset;
    int isGenerated;
    int isMinimal;
    ...
    if(isValid)
    {
        ...
        else
        {
            ...
            if(strcmp((char*)protocol_name,"beidouchain") == 0)
            {
...
int mc_BeidouchainParams::Print(FILE* fileHan)
{
    int i,c,size;
    int version;
    int header_printed;
    ...
    fprintf(fileHan,"# ==== BeidouChain configuration file ====\n\n");
    fprintf(fileHan,"# Created by beidouchain-util \n");
    ...
    switch(m_Status)
    {
        case MC_PRM_STATUS_EMPTY:
            ...
            fprintf(fileHan,"# To join network please run \"beidouchaind %s@<seed-node-ip>[:<seed-node-port>]\".\n",Name());
            return MC_ERR_NOERROR;
        case MC_PRM_STATUS_ERROR:
            ...
            fprintf(fileHan,"# Please fix it and rerun beidouchain-util. \n");
            break;
        ...
        case MC_PRM_STATUS_GENERATED:
            ...
            fprintf(fileHan,"# To generate network please run \"beidouchaind %s\".\n",Name());
            break;
        case MC_PRM_STATUS_VALID:
            fprintf(fileHan,"# This parameter set is VALID. \n");
            fprintf(fileHan,"# To join network please run \"beidouchaind %s\".\n",Name());
            break;
    }
        ...
        while(i>=0)
        {
                    ...
                    ((m_lpParams+i)->IsRelevant(version) > 0))
            {
                if(header_printed == 0)
                {
                    ...
                    {
                        case MC_PRM_COMMENT:
                            fprintf(fileHan,"# The following parameters don't influence beidouchain network configuration. \n");
                        ...
                        case MC_PRM_USER:
                            if(m_Status == MC_PRM_STATUS_ERROR)
                            {
                                ...
                                if(Name())
                                {
                                    fprintf(fileHan,"# Please rerun \"beidouchain-util clone %s <new-network-name>\". \n",Name());
                                }
                            }
                            else
                            {
                                ...
                                {
                                    fprintf(fileHan,"# The following parameters can be edited before running beidouchaind for this chain. \n");                                    
                                }
                                else
                                {
                                    fprintf(fileHan,"# The following parameters can only be edited if this file is a prototype of another configuration file. \n");
                                    fprintf(fileHan,"# Please run \"beidouchain-util clone %s <new-network-name>\" to generate new network. \n",Name());
                                }
                            }
                            break;
                        case MC_PRM_GENERATED:
                            fprintf(fileHan,"# The following parameters were generated by beidouchain-util.\n");
                            fprintf(fileHan,"# They SHOULD ONLY BE EDITED IF YOU KNOW WHAT YOU ARE DOING. \n");
                            break;
                        case MC_PRM_CALCULATED:
                            fprintf(fileHan,"# The following parameters were generated by beidouchaind.\n");
...
int mc_BeidouchainParams::Write(int overwrite)
{
    FILE *fileHan;
    int create;
...
const char* mc_BeidouchainParams::Name()
{
    return (char*)GetParam("chainname",NULL);
}

const unsigned char* mc_BeidouchainParams::MessageStart()
{
    return (unsigned char*)GetParam("networkmessagestart",NULL);
}

const unsigned char* mc_BeidouchainParams::DefaultMessageStart()
{
    return c_DefaultMessageStart;
}


const unsigned char* mc_BeidouchainParams::AddressVersion()
{
    return (unsigned char*)GetParam("addresspubkeyhashversion",NULL);
}

const unsigned char* mc_BeidouchainParams::AddressScriptVersion()
{
    return (unsigned char*)GetParam("addressscripthashversion",NULL);
}

const unsigned char* mc_BeidouchainParams::AddressCheckumValue()
{
    return (unsigned char*)GetParam("addresschecksumvalue",NULL);
}


int mc_BeidouchainParams::ProtocolVersion()
{
...
int mc_BeidouchainParams::IsProtocolBeidouchain()
{
    return m_IsProtocolBeidouChain;
}
...
int mc_Features::VerifySizeOfOpDropElements()                                   // This test is still in the code to keep protocol!-beidouchain untouched
{
    
    int ret=0;        
    int protocol=mc_gState->m_NetworkParams->ProtocolVersion();
    
    if(mc_gState->m_NetworkParams->IsProtocolBeidouchain() == 0)
    {
        return 1;
    }
