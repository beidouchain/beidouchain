...
#ifndef BEIDOUCHAINPARAMS_H
#define	BEIDOUCHAINPARAMS_H
...
typedef struct mc_OneBeidouchainParam
{    
...
} mc_OneBeidouchainParam;

typedef struct mc_BeidouchainParams
{    
    char *m_lpData;
    mc_MapStringIndex *m_lpIndex;
    mc_OneBeidouchainParam *m_lpParams;
    int *m_lpCoord;
    int m_Status;
    int m_Size;
    int m_Count;
    int m_IsProtocolBeidouChain;
    int m_ProtocolVersion;
    
    int m_AssetRefSize;
    
    mc_BeidouchainParams()
    {
        Zero();
    }
    ...
    int Clone(const char *name,mc_BeidouchainParams *source);
